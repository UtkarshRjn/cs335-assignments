...
class ModelClass(torch.nn.Module):
    ...
    def loss(self, pred, label):
        ...
        if self.args.model_type == "svm": 
            ##### TODO: Your code starts here#########
            label = label.reshape([pred.shape[0],1])
            # print
            new_label = torch.where(label == 0, -1 , 1)
            temp = 1 - torch.multiply(pred, new_label)
            temp[temp < 0 ] = 0  
            ...
            ##### TODO: Your code starts here######
            label = label.reshape([pred.shape[0],1])  
            loss = -torch.mean(torch.multiply(label, torch.log(pred)) + torch.multiply(1-label, torch.log(1-pred)))     
            ######Your code ends here##########
        elif  self.args.model_type == "ranking":
            ##### TODO: Your code starts here######  
            P = pred[label == 1]
            ...
            n1 = len(P)
            n2 = len(N)
            N2Vec = torch.ones((n2,1)).T
            N1Vec = torch.ones((n1,1))
            MatP = torch.matmul(P.reshape(n1,1),N2Vec)
            MatN = torch.matmul(N1Vec,N.reshape(n2,1).T)
            loss = torch.sum(torch.maximum(torch.zeros(MatP.shape), MatN - MatP))
            ######Your code ends here##########
        else: 
            raise NotImplementedError()
...
def evaluate(loader, model):
    ...
    for data in loader:
        with torch.no_grad():
            pred = model(data[0])
            label = data[1].reshape([pred.shape[0],1])
            if model.args.model_type == "svm": 
                ...
                eval_score += torch.sum(torch.eq(pred, label))         
                ###### Your code ends here##########
                ...
                if(i == 0): 
                    ...
                    # print(Pmat.shape)
                    LongP = torch.cat((LongP,Pmat), 0)
                    LongN = torch.cat((LongN,Nmat), 0)
    ...
    if(model.args.model_type != "ranking"): eval_score = eval_score.item() / N
    else: 
        n1 = len(LongP)
        n2 = len(LongN)
        N2Vec = torch.ones((n2,1)).T
        N1Vec = torch.ones((n1,1))
        MatP = torch.matmul(LongP.reshape(n1,1),N2Vec)
        MatN = torch.matmul(N1Vec,LongN.reshape(n2,1).T)
        eval_score = torch.sum(torch.where(MatP > MatN, 1, 0))
