...
def create_weights(data_dictionary, lambda_val):
    ...
    weights = np.dot( np.dot( np.linalg.inv( np.dot(X_train.T,X_train) + lambda_val*np.identity(X_train.shape[1]) ), X_train.T) ,Y_train)
...
def generate_test_error(data_dictionary, weights):
    ...
    test_error = np.mean(np.square(np.dot(X_test,weights) - Y_test))
...
if __name__=="__main__":
    ...
    for lmbd in range(100):
        my_lmbd = lmbd*0.3/100
        weights = create_weights(data_dictionary=data_dictionary,lambda_val=my_lmbd)
        err = generate_test_error(data_dictionary=data_dictionary, weights=weights)
        error_array.append(err)
        lambda_array.append(my_lmbd)
