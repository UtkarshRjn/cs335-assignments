...
class LogisticRegression:
    ...
    def backward(self, batch_x, batch_y, batch_yhat):
        ...
        ## TODO
        weights_new = self.weights - self.args.lr * torch.mean( torch.mul((batch_yhat - batch_y) , batch_x.T).T, dim=0) * self.args.temp
        bias_new = self.bias - self.args.lr * torch.mean(batch_yhat - batch_y) * self.args.temp
        ## End TODO
...
class LinearRegression:
    ...
    def backward(self, batch_x, batch_y, batch_yhat):
        ...
        ## TODO
        weights_new = self.weights - self.args.lr * torch.matmul(batch_x.T, (batch_yhat - batch_y))/ batch_y.shape[0]
        bias_new = self.bias - self.args.lr * torch.mean(batch_yhat - batch_y)
        ## End TODO
